...
public class PhoneInfo
{
	...
	public static boolean checkNetWork(Context paramContext)
	{
		 ...
		 * if (localConnectivityManager == null); NetworkInfo[]
		 * arrayOfNetworkInfo; do { return false; arrayOfNetworkInfo =
		 * localConnectivityManager.getAllNetworkInfo(); } while
		 * (arrayOfNetworkInfo == null); int i = arrayOfNetworkInfo.length; int
		 * j = 0; if (j < i) { NetworkInfo localNetworkInfo =
		 * arrayOfNetworkInfo[j]; if (localNetworkInfo != null) {
		 * NetworkInfo.State localState = localNetworkInfo.getState(); if
		 * ((localState == null) || ((localState != NetworkInfo.State.CONNECTED)
		 * && (localState != NetworkInfo.State.CONNECTING))); } } for (int k =
		 * 1; ; k = 0) { return k; j++; break; }
		 */
	}
	...
	public static String getSerialNum()
	{
		try
		{
			...
			String str = (String) localClass.getMethod("get", new Class[]
			{ String.class, String.class }).invoke(localClass, "ro.serialno", "unknown");
			return str;
	...
	{
		for (NetworkInfo localNetworkInfo : ((ConnectivityManager) paramContext.getSystemService("connectivity"))
				.getAllNetworkInfo())
		{
